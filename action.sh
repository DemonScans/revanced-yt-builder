...
patch_apk() {
	...
	local cmd="java -jar $rv_cli_jar --path options.json patch -b $rv_patches_jar -m $rv_integ_apk -o $patched_apk -p --rip-lib x86_64 --rip-lib x86 --rip-lib armeabi-v7a --keystore=ksdef.keystore $patcher_args $stock_input"
	pr "$cmd"
	if [ "${DRYRUN:-}" = true ]; then
